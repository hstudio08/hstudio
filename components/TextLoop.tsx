...
type ShapeType = 'wave' | 'circle' | 'infinity' | 'arch' | 'line';
type DirectionType = 'forward' | 'reverse';

const buildPath = (shape: ShapeType, curviness: number, ribbonWidth: number, viewH: number) => {
  const CY = viewH / 2;
...
export default function TextLoop({
  ...
  const viewH = useMemo(() => {
    if (shape === 'line') return Math.max(50, ribbonWidth + 16);
    return DEFAULT_VIEW_H;
  }, [shape, ribbonWidth]);
  ...
  }, [d, unit, fontSize, fontWeight, letterSpacing, shape]);
  ...
  }, [metrics, speed, direction, pauseOnHover, shape]);

  // HIGH-PRECISION RESPONSIVE TICKER FOR LINE SHAPE (PREVENTS SVG SCALE DOWN ON MOBILE)
  if (shape === 'line') {
    return (
      <div
        ref={rootRef}
        className={`w-full overflow-hidden relative select-none pointer-events-auto py-3 sm:py-4 border-y border-slate-200/80 shadow-xs ${className}`.trim()}
        style={{
          backgroundColor: ribbon ? ribbonColor : 'transparent',
          ...style
        }}
      >
        <div className="animate-text-loop-marquee">
          <span className="inline-block px-2 font-black uppercase text-[#0b1c30] text-sm sm:text-base md:text-lg tracking-widest">
            {unit}{unit}
          </span>
          <span className="inline-block px-2 font-black uppercase text-[#0b1c30] text-sm sm:text-base md:text-lg tracking-widest">
            {unit}{unit}
