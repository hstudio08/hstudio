...
// IMPORTANT: Adjust this path to wherever you saved the ElectricBorder component!
...
export default function Pricing() {
  const matrices = [
    ...
    },
    {
      ...
      features: ["Upto 10 Pages", "Premium Design", "Advanced SEO", "Custom Features", "1-2 Revisions", "10 days Priority Support (Call/Whatsapp/Email)", "Contact Form", "Admin Panel", "1 month bug fixes", "Upto 150 Products/Services (Expandable)", "Payment Gateway Integration", "Blog/News Section", "Analytics Setup", "Social Media Integration"]
    }
  ];
    ...
    <section id="packages" className="py-16 md:py-24 bg-slate-50/40 relative z-10">
      <div className="max-w-[96rem] mx-auto px-4 sm:px-6 md:px-8">
        ...
        <div className="text-center flex flex-col items-center space-y-2 md:space-y-3 mb-10 md:mb-16">
          ...
          <h2 className="text-3xl sm:text-4xl md:text-6xl font-black tracking-tight text-slate-950 font-['Foldit'] drop-shadow-sm px-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-10 items-stretch">
          {matrices.map((tier, idx) => {
            ...
            // Abstracted the inner content so we don't duplicate code between the standard div and ElectricBorder wrapper
            const cardInnerContent = (
              <>
                <div className="relative z-10">
                  {/* Conditional Popular Float Badge */}
                  {tier.popular && (
                    <span className="absolute top-0 right-0 md:right-1/2 md:translate-x-1/2 -translate-y-[150%] md:-translate-y-1/2 bg-slate-950 text-[#b4f7ab] font-black text-[10px] uppercase tracking-wider px-4 py-1.5 rounded-full shadow-md font-['Familjen_Grotesk'] z-30">
                      Most Popular
                    </span>
                  )}
            ...
            // If it's the premium tier, wrap it in the ElectricBorder
            if (tier.popular) {
              return (
                <ElectricBorder
                  key={idx}
                  color="#2563eb" // A rich blue that complements the light green/cyan card
                  speed={1.5}
                  chaos={0.12}
                  borderRadius={32} // 32px matches Tailwind's rounded-[2rem]
                  className={`rounded-[2rem] transition-all duration-300 h-full w-full ${tier.cardStyle}`}
                >
            ...
            // Standard render for Starter and Growth packages
            return (
              <div 
                key={idx}
                className={`border rounded-[2rem] p-6 md:p-8 flex flex-col justify-between relative transition-all duration-300 overflow-hidden h-full w-full ${tier.cardStyle}`}
              >
                {cardInnerContent}
              </div>
            );
